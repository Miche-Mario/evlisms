    ...
    const displaySteps = newStep.map((step, index) => {
        return (
            ...
            className={index !== newStep.length -1 ? 
                ...
                <div className='relative flex flex-col items-center text-green-300'>
                    ...
                    text-xs font-medium uppercase ${step.highlighted ? 
                    "text-gray-900" : "text-gray-400"}`}>
                        {step.description}
                    </div>
                </div>
                <div className={`flex-auto border-t-2 transition duration-500
                ease-in-out ${step.completed ? "border-green-600" :
                "border-gray-300"}`}>
                   {/*  display line */}
                </div>
            </div>
