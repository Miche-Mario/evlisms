...
const EditLanguage = ({props}) => {
  ...
  const getLanguage = async () => {
    const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/language/${id}`);
  ...
  const updateLanguage = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`${process.env.REACT_APP_BASE_URL}/language/${id}`, {
  ...
  return (
    <Layout>
        <div className='mt-10 ml-5'>
            ...
            <p className='text-gray-400 text-2xl'>Edit Language</p>
            <div className='bg-white h-[20rem] p-5  ml-1 mt-3 elevation'>
                <form onSubmit={updateLanguage}>
                  
                  
                    <div className='mt-5'>
                      <label className='text-xl font-bold'>Lanuage</label>
