...
const EditPaymentMethod = ({props}) => {
  ...
  const getMethod = async () => {
    const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/paymentmethod/${id}`);
    setPaymentname(response.data.paymentname);

  }
  const updatePaymentMethod = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`${process.env.REACT_APP_BASE_URL}/paymentmethod/${id}`, {
  ...
  return (
    <Layout>
        <div className='mt-10 ml-5'>
            ...
            <p className='text-gray-400 text-2xl'>Edit Payment Method</p>
